...
namespace ELibrary.Repositories
{
    ...
    {
        public BorrowingRepository(ELibraryContext context)
            : base(context) { }

        public async Task<IPagedList<Borrowing>> GetBorrowingDetails(
            string? search,
            int pageNumber,
            int pageSize = 15
        )
        {
            ...
            {
                query = query.Where(b =>
                    b.Member.MemberNumber.ToLower().Contains(search.ToLower())
