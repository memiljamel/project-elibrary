...
namespace ELibrary.Repositories
{
    ...
    {
        public MemberRepository(ELibraryContext context)
            : base(context) { }

        public async Task<IPagedList<Member>> GetPagedMembersWithPhones(
            string? search,
            int pageNumber,
            int pageSize = 15
        )
        {
            var query = _context.Members.Include(m => m.Phones).AsQueryable();

            if (!string.IsNullOrEmpty(search))
            {
                query = query.Where(m =>
                    m.MemberNumber.ToLower().Contains(search.ToLower())
